...
public:
  ...
  void print_help() {
    for (std::map<std::string, std::string>::const_iterator pv = help.begin();
         pv != help.end(); ++pv) {
      std::cout << "-" << pv->first;
      for (int i = pv->first.size() + 1; i < 16; i++) {
        std::cout << " ";
      }
  ...
  const std::string &registerParameter(const std::string &parameter,
  ...
  void checkParameters() {
    ...
    // registered:
    for (std::map<std::string, std::string>::const_iterator pv = value.begin();
         pv != value.end(); ++pv) {
