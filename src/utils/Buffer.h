...
public:
  ...
  explicit BasicBuffer(BasicBuffer &&other)
      : _buffer(other._buffer), _cursor(other._cursor), _end(other._end),
  ...
  */
  ...
  char *cursor() const { return _cursor; }
   ...
   */
  size_t read_size() const { return cursor() - buffer(); }
  ...
  void set_buffer(char *x) { _buffer = x; }
  void set_cursor(char *x) {
    CHECK(_cursor < end());
    _cursor = x;
  }
  void set_end(char *x) { _end = x; }
  /*
   * put the cursor to the begin of buffer
   */
  void reset_cursor() { _cursor = buffer(); }
  /*
...
protected:
  ...
  // used in write mod
  void end_preceed(size_t size) { _end += size; }
...
protected:
  ...
  template <typename T> void get_raw(T &x) {
    CHECK(!read_finished());
   ...
   */
  template <typename T> void put_raw(T &x) {
