 ...
 *  };
 */
template <typename T> class BasicChannel : public threadsafe_queue<T> {
public:
  ...
  */
  bool pop(T &value) {
    ...
    data_cond.wait(lk, [this] { return !data_queue.empty() || _closed; });
    if (_closed) {
      value = T();
      return false;
    }
  ...
  */
  ...
  size_t size() const { return data_queue.size(); }
