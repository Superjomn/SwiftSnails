...
typedef std::function<void(int shared_id, BinaryBuffer &ibb, BinaryBuffer &obb)>
    ShardCallBack;
...
class Message {
public:
  Message() { PCHECK(0 == zmq_msg_init(&_zmg)); }
  ...
  Message(BasicBuffer &&bb) {
    PCHECK(0 ==
           zmq_msg_init_data(&_zmg, bb.buffer(), bb.size(), self_free, NULL));
  ...
  ~Message() { PCHECK(0 == zmq_msg_close(&_zmg)); }
  ...
  size_t size() { return zmq_msg_size(&_zmg); }
  ...
  char *buffer() { return (char *)zmq_msg_data(&_zmg); }

  zmq_msg_t &zmg() { return _zmg; }
  /*
...
struct Request {
  ...
  int message_id() const { return meta.message_id; }

  void set_msg_id(int id) { meta.message_id = id; }
  // to tell whether this message
  // is a request from other node
  // or response from the receiver
  bool is_response() const { return meta.message_class == -1; }
  void set_response() { meta.message_class = -1; }
  /*
   * set node's type: worker / server
   */
  void set_worker() { meta.client_id = -1; }
  void set_server() { meta.client_id = -2; }
  bool is_worker() { return meta.client_id == -1; }
  bool is_server() { return meta.client_id == -2; }
