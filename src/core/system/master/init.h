...
public:
  explicit MasterTransferInit() {
    ...
    expected_node_num = global_config()
                            .get_config("expected_node_num")
                            .to_int32(); // nodes include server and worker
  ...
  void operator()() {
    ...
    CHECK(!gtransfer.async_channel()->closed())
        << "channel should not been closed before transfer is deconstructed";
  ...
  int worker_num() { return registered_node_num - registered_server_num; }
  int server_num() { return registered_server_num; }
...
protected:
  ...
  void send_route_to_workers() {
    ...
    LOG(INFO) << "get \t" << registered_node_num - registered_server_num
              << "\tworkers";
  ...
  void wait_to_terminate() {
    int timeout =
        global_config().get_config("master_longest_alive_duration").to_int32();
...
protected:
  ...
  transfer_t::msgcls_handler_t node_init_address = [this](
    ...
    int id =
        gtransfer.route().register_node_(request->is_server(), std::move(addr));
    // cache message_ids
  ...
  transfer_t::msgcls_handler_t node_askfor_hashfrag = [this](
      std::shared_ptr<Request> request, Request &response) {
