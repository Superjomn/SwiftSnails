...
template <typename T, typename Func> class ThreadPool : public VirtualObject {
public:
  typedef BasicChannel<T> channel_t;

  explicit ThreadPool() {}
  explicit ThreadPool(int num_thread) : _thread_num(num_thread) {}
  ~ThreadPool() {
    for (auto &t : _threads) {
      if (t.joinable())
        t.join();
  ...
  std::shared_ptr<channel_t> start(Func &&func) { // TODO queue to exit
    ...
    return {&_channel, [this](channel_t *) {
