...
public:
    ...
    void pull() {
        RAW_LOG(INFO, "minibatch pull %lu keys", local_keys.size());
        CHECK(! local_keys.empty());
        pull_access.pull_with_barrier(local_keys, global_cache.cache());
    }

    void push() {
        RAW_LOG(INFO, "minibatch push %lu keys", local_keys.size());
        if(local_keys.empty()) {
            RAW_LOG(INFO, "local_keys empty, do not push");
            return;
        }
	...
	void TrainModelThread(const string &train_file, int id, Error& global_error) {
		...
		while (true) {
            ...
            if(sentence_counter > 0 && sentence_counter % num_sents_to_cache == 0 && sentence_counter != last_sentence_counter) {
                // push last minibatch
                RAW_LOG(INFO, "sentence_counter:\t%d", sentence_counter);
