...
public:
    ...
    void pull() {
        LOG(INFO) << "to pull from PS";
        CHECK(! local_keys.empty());
        LOG(INFO) << "pull " << local_keys.size() << " keys";
        pull_access.pull_with_barrier(local_keys, param_cache);
        LOG(INFO) << "get pulled param " << param_cache.size();
    }

    void push() {
        LOG(INFO) << "to push to PS";
        if(local_keys.empty()) {
...
/*
	void TrainModelThread(const string &train_file, int id, Error& global_error) {
        ...
        LOG(INFO) << "to init Vec neu1 and neu1e";
        Vec neu1(len_vec);
        Vec neu1e(len_vec);
        ...
        LOG(INFO) << "file_size: " << file_size;
        LOG(INFO) << "to fseek task for this thread";
		fseek(fi, file_size / (long long) num_threads * (long long) id, SEEK_SET);
        LOG(INFO) << "after seek task";
		while (1) {
			...
			if (cbow) {  //train the cbow architecture
				...
				for (a = b; a < window * 2 + 1 - b; a++)
					if (a != window) {
						...
						if (last_word == -1)
							continue;
                        Vec& syn0_last_word = global_param.cache().params()[last_word].v();
						for (c = 0; c < len_vec; c++)
                            neu1[c] += syn0_last_word[c];
				...
				if (negative > 0)
					for (d = 0; d < negative + 1; d++) {
						if (d == 0) {
							...
							target = vocab.table[(next_random >> 16) % table_size];
							if (target == 0)
						...
						//l2 = target * len_vec;
                        //RAW_LOG(INFO, ">  update syn1neg_target");
				...
				for (a = b; a < window * 2 + 1 - b; a++)
					if (a != window) {
                        ...
                        Vec& syn0_last_word = global_param.cache().params()[last_word].v();
						for (c = 0; c < len_vec; c++)
                            syn0_last_word[c] += neu1e[c];
							//syn0[c + last_word * len_vec] += neu1e[c];
                        //syn0_last_word += neu1e;
					}
			}
			sentence_position++;
